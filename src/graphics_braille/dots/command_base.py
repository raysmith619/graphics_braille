# command_base.py    18May2025  crs  from dots-master/src/select_command.py
...
class CommandBase:
    ...
    command_manager = None      # Must be set via cls.set_manager()

    @classmethod
    def set_manager(cls, manager):
        """ Setup link to manager
        :manager:  Setup link to manager
        """
        cls.command_manager = manager
    ...
    def __init__(self, action_or_cmd, cmd_num=None,
        ...
        if self.command_manager is None:
            raise SelectError("No CommandBaseManager")
    ...
    def do_cmd(self):
        ...
        self.command_manager.displayPrint(
                f"do_cmd[{self.cmd_num}]({self.action})", "execute")
        self.command_manager.selectPrint("do_cmd({self.action}", "execute")
        self.command_manager.cmd_stack_print("do_cmd({self.action}", "execute")
        res = self.execute()
        if res:
...
if __name__ == '__main__':
    ...
    SlTrace.lg(f"dir: {os.path.dirname(__file__)}")
