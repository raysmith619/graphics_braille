...
# The following implementation of custom RQueue to avoid NotImplementedError
...
class RQueue(Queue):
    ...
    serialize RQueue when it is passed between processes. If these functions
    are not defined, RQueue cannot be serialized, which will lead to the error
    of "AttributeError: 'RQueue' object has no attribute 'size'".
    ...
    def __init__(self, size=None):
        """ Setup r obust queue
        :size: max queue size - NOT implemented/used
        """
        self.max_size = size
    ...
    def __getstate__(self):
        """Help to make RQueue instance serializable.

        Note that we record the parent class state, which is the state of the
        actual queue, and the size of the queue, which is the state of RQueue.
