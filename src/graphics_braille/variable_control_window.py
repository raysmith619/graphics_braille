...
from graphics_braille.select_control import SelectControl
from tkinter.constants import TOP

class VariableControlWindow(Toplevel):
    def __init__(self, master=None, window_exit=None, var_ctl=None, update_call=None):
        ...
        """
        SlTrace.lg("VariableControlWindow")
        if master is None:
            master = Toplevel()            
            SlTrace.lg("VariableControlWindow - new master")
        self.vc_mw = master
        ...
        for var_name in self.var_ctl.get_var_names(label=True):
            ...
            var_label = self.var_ctl.get_var_label(var_name)
            val = self.var_ctl.get_val(var_name)
            SlTrace.lg(f"VariableControlWindow: {var_name} var_label:'{var_label}' entry_type:{entry_type} val:'{val}'")
            if entry_type == "internal":
                continue
            if entry_type == "label":
    ...
    def destroy(self):
        """ Destroy us
        """
        SlTrace.lg("VariableControlWindow.destroy")
        self.delete_vc_window()
            
    def delete_vc_window(self):
        """ Process Trace Control window close
        """
        SlTrace.lg("VariableControlWindow.delete_vc_window")
    ...
    def select_button(self, event):
        SlTrace.lg(f"VariableControlWindow.select_button {event}")
    ...
    def set_val(self, name, value, no_change=False):
        ...
        :no_change: don't 
        """
        SlTrace.lg(f"VariableControlWindow set_val name:{name}, value={value}")
    ...
    def var_update(self):
        ...
        """
        SlTrace.lg(f"VariableControlWindow.var_update")
    ...
    def var_undo(self):
        ...
        """
        SlTrace.lg(f"VariableControlWindow.var_undo")
    ...
    def var_redo(self):
        """ undo last var_undo
        """
        SlTrace.lg(f"VariableControlWindow.var_redo")
...
if __name__ == '__main__':
    ...
    def set_controls():
        ...
        if vC is not None:
            vC.destroy()
            vC = None
        vC = VariableControlWindow(var_ctl=cF, update_call=update_call)
